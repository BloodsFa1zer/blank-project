...
Автоматично виконує всі операції: валідацію, парсинг всіма трьома парсерами, сортування та XSL трансформацію.
**Запускається автоматично** при старті програми без аргументів:
...
#### Інтерактивний режим
Дозволяє вибрати конкретну операцію для виконання. Запускається з аргументом "interactive":

```bash
mvn exec:java -Dexec.mainClass="main.Main" -Dexec.args="interactive"
```
...
В інтерактивному режимі доступні наступні опції:
- 1 - Валідація XML
- 2 - Парсинг через SAX
- 3 - Парсинг через DOM
- 4 - Парсинг через StAX
- 5 - XSL трансформація
- 6 - Порівняння всіх парсерів
- 7 - Сортування каменів
- 8 - Виконати все (як в демо)
- 0 - Вихід
